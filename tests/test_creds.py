...
def test_get_etype_info(realm: k5test.K5Realm, tmp_path: pathlib.Path) -> None:
    ...
    # Get information about enctype, salt and s2kparams for the principal
    enctype, salt, s2kparams = krb5.get_etype_info(ctx, princ)
    ...
    kb = krb5.c_string_to_key(ctx, enctype, realm.password("user").encode(), salt, s2kparams)
