...
def test_cc_retrieve_remove_cred(realm: k5test.K5Realm, tmp_path: pathlib.Path) -> None:
    ...
    msg_pattern = "Matching credential not found|End of credential cache reached"
    ...
    if realm.provider.lower() == "heimdal":
        pytest.skip("Removing credentials does not seem to have an effect with heimdal")
